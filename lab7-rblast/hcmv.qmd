...
The ratio in cycles 1-10 is about A=25% C=30% G=20 T=20 and the ratios for cycles 65-75 is abut A=25% C=27% G=23% T=25%. This is probably because of low sequence diversity at the beginning of reads and as the sequencing continues it is moved into random genomic DNA fragments.
...
The typical quality of cycles 1-10 is about 35-40 and the typical quality of cycle 65-75 is about 20-30. This is probably because in early cycles signal detection is strong and clean and as you continue in the cycles the phasing errors build up and signal intensity decreases.
...
seqs <- readDNAStringSet("hcmv15-hcmv1b-1.fasta")
 seqs <- DNAString(toupper(as.character(seqs)))
 
 gap_positions <- gregexpr("-", as.character(seqs))
 
 allignseqs <- matchPattern("-", seqs, max.mismatch=20)
 
 find the 187075th letter and start there
 
 countseqs <- countPattern("-", seqs, max.mismatch = 5)
 
 refseqs <- DNAStringSet(gsub("-", "", as.character(seqs)))
```
